...
export default function HeroTab({ settings, updateSetting, toggleItem, deleteItem, addItem, handleFileUpload, setPendingFile, pendingFiles }) {
  const bgPending = pendingFiles?.hero?.heroSectionBgImage || pendingFiles?.heroBgImage

  const onBgFile = (file) => {
    if (!setPendingFile) return
    setPendingFile('hero', {
      heroSectionBgImage: file,
    })
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <DualImageInput
        ...
        onFileSelected={onBgFile}
        pendingFile={bgPending}
      />
