                  ...
                  {lowStockProducts.map((product) => (
                    <li key={product._id} className="flex items-center gap-3 p-2 rounded bg-soft-cream/50">
                      <div className="w-8 h-8 bg-surface-container rounded overflow-hidden shrink-0">
                        <img className="w-full h-full object-cover" alt={product.name} src={resolveImageUrl(product.primaryImage || product.images?.[0] || '')} onError={(e) => { e.target.src = 'https://placehold.co/48x48' }} />
                      </div>
                      <div className="flex-1 min-w-0">
