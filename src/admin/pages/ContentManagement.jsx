...
export default function ContentManagement() {
  ...
  const setPendingFile = (field, file) => {
    setPendingFiles((prev) => ({ ...prev, [field]: file }))
  }

  const clearPendingFile = (field) => {
  ...
  const handleSaveTab = async () => {
    ...
    try {
      ...
      let response
      if (pendingFile) {
        response = await contentAPI.updateHomepageTabWithUpload(activeTab, settings, pendingFile)
      } else {
        response = await contentAPI.updateHomepageTab(activeTab, settings)
      }
      if (response.data.success) {
        setSettings(response.data.data)
        clearPendingFile(activeTab)
