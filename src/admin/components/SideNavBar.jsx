...
export default function SideNavBar({ isOpen, onClose }) {
  ...
  return (
    <aside
      className={`bg-white border-r border-gray-200 flex flex-col h-screen z-30 transition-transform duration-300 ease-in-out fixed left-0 top-0 ${
            ...
            >
              <i
                className={`${item.icon} w-5 text-center text-sm ${
                  isActive
                    ? 'opacity-90'
                    : 'opacity-70 group-hover:opacity-100 group-hover:text-regal-gold'
                }`}
              ></i>
